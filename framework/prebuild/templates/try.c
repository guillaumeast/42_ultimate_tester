do
{
	...
	if (test.type != COMPARE_OUTPUTS) test.fn1.ret = (intptr_t)((int (*)(const char *, ...))test.fn1.fn)(__VA_ARGS__);
	if (test.type != COMPARE_RETURNS) redirect_read(&(test.fake_stdout), &(test.fn1.out));
	if (test.type != COMPARE_OUTPUTS) test.fn2.ret = (intptr_t)((int (*)(const char *, ...))test.fn2.fn)(__VA_ARGS__);
